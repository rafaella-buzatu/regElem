...
In this project, we make use of the software **cisTopic** (Bravo González-Blas et al., 2019), a framework employing the latent Dirichlet allocation (LDA) (Blei et al., 2003), that allows us to co-cluster DNA regulatory regions and cell types. 
...
```{r}
ATACseq_data = readRDS('ATACseq_data/Zhang_BICCN-H_20190523-20190611_huMOp_Final_AC_Peaks.RDS')

#Subset if needed
#ATACseq_data = ATACseq_data[1:2000,1:2000]
...
#Plot Umap; color by given attributes
...
```
Following this, we can generate a BigWig file to be able to visualise the scored regions in the genome.
```{r}
